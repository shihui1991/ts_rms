...
@section('content')
    ...
    <form class="form-horizontal" role="form" action="{{route('g_landsource_add')}}" method="post">
        {{csrf_field()}}

        <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right" for="landprop"> 土地性质： </label>
            <div class="col-sm-9">
                <input type="text" id="landprop" value="{{$sdata->name}}" class="col-xs-10 col-sm-5" readonly>
                <input type="hidden" name="prop_id" value="{{$sdata->id}}">
            </div>
        </div>
