...
<div id="tip" style="width: 794px;height: auto;margin: auto;">
    ...
    <div class="content" style="width: 794px;margin: auto;">
        <h1 class="center_text" style="line-height: 120px;">房屋征收补偿安置协议书(一)</h1>
        <div>
        ...
        <p class="color66" style="line-height: 50px;text-indent: 50px;">根据《国有土地上房屋征收与补偿条例》、《甘肃省实施
            &lt;国有土地上房屋征收与补偿条例&gt;若干规定》、《国有土地上房屋征收评估办法》及《天水市秦州区人民政府关于{{$item->name}}房屋征收补偿安置实施方案》的规定，甲乙双方本着公平合理、平等协商的原则，就乙方位于该项目规划区域内房屋的征收补偿安置达成如下协议。</p>
        <!--这里是协议内容-->
        <div>
            <!--无列表样式-->
            <div>
                ...
                <div class="text_indent50 color66">
                    房屋所在地<div class="div_border_bot">
                    ...
                    ，证载建筑面积<div class="div_border_bot">{{number_format($household_detail->reg_outer,2)}}</div>平方米
                    ，实测面积<div class="div_border_bot">{{number_format($real_area,2)}}</div>平方米
                    ，房屋所有权人<div class="div_border_bot">
                    ...
                    ，房屋产权证号<div class="div_border_bot">{{$household_detail->register}}</div>
                    ，房屋的批准用途<div class="div_border_bot">{{$household_detail->defbuildinguse->name}} </div>
                    ，房屋的实际用途<div class="div_border_bot">{{$household_detail->realbuildinguse->name}} </div>
                    ，房屋的产权性质<div class="div_border_bot">{{$household->type}} </div>
                    ，房屋的结构<div class="div_border_bot">{{$household->itembuilding->buildingstruct->name}}</div>
                    ，房屋的总层数<div class="div_border_bot">{{$household->itembuilding->total_floor}}</div>
                    ，房屋所在楼层<div class="div_border_bot">{{$main_building->floor}}</div>
                    ，房屋朝向<div class="div_border_bot">{{$main_building->direct}}</div>。
                </div>
            </div>
            <div>
                ...
                <div class="text_indent50 color66">
                    <div class="text_indent50 ">
                        乙方选择以下的<div class="width100 div_border_bot">“{{$pay->repay_way}}”</div>补偿方式
                    </div>
                    <ul>
                                ...
                                @if(filled($register_buildings))
                                    @foreach($register_buildings as $building)
                                        <tr>
                                            ...
                                            @if($household->getOriginal('type'))
                                                <td>{{number_format($building->amount*$program->portion_renter/100,2)}}</td>
                                            @else
                                                <td>{{number_format($building->amount,2)}}</td>
                                            @endif
                                        </tr>
                            ...
                            <table class="table_a color66" border="1" cellspacing="0" style="width: 100%;">
                                ...
                                @if(filled($legal_buildings))
                                    @foreach($legal_buildings as $building)
                                        <tr>
                                            <td>{{$building->buildingstruct->name}}</td>
                                            <td>{{$building->householdbuilding->build_year}}</td>
                                            <td>{{number_format($building->real_outer,2)}}</td>
                                            <td>{{number_format($building->price,2)}}</td>
                                            <td>{{number_format($building->amount,2)}}</td>
                                        </tr>
                                ...
                                @if(filled($pay_objects))
                                    @foreach($pay_objects as $object)
                                        <tr>
                                            ...
                                            <td>{{number_format($object->number,2)}}</td>
