...
@section('content')
    ...
    <form class="form-horizontal" role="form" action="{{route('g_itembuilding_edit')}}" method="post">
        ...
        <div class="form-group">
            ...
            <label class="col-sm-3 control-label no-padding-right" for="land_id"> 地块地址： </label>
            <div class="col-sm-9">
                <select class="col-xs-5 col-sm-5" name="land_id" id="land_id">
                    <option value="">--请选择--</option>
                    @if($edata['itemland'])
                        @foreach($edata['itemland'] as $itemland)
                            <option value="{{$itemland->id}}" @if($itemland->id == $sdata->land_id) selected @endif>{{$itemland->address}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
