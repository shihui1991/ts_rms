...
@section('content')
    ...
    <form class="form-horizontal" role="form" action="{{route('g_funds_out',['item'=>$sdata['item']->id])}}" method="post">
        {{csrf_field()}}

        <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right" for="cate_id"> 进出类型： </label>
            <div class="col-sm-9">
                <select name="cate_id" id="cate_id" class="col-xs-10 col-sm-5">
                    <option value="">--请选择进出类型--</option>
                    <option value="7">项目拆除费</option>
                    <option value="8">项目评估费</option>
                </select>
            </div>
        </div>
