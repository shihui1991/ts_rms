...
@section('content')
    ...
    <form class="form-horizontal" role="form" action="{{route('g_pact_add2',['item'=>$sdata['item']->id,'pay_id'=>$sdata['pay']->id])}}" method="post">
        {{csrf_field()}}

        <div class="form-group">
            <label class="col-sm-3 control-label no-padding-right" for="sign_date"> 签约日期： </label>
            <div class="col-sm-9">
                <input type="text" id="sign_date" name="sign_date" value="{{old('sign_date')}}" class="col-xs-10 col-sm-5 laydate"  required>
            </div>
        </div>
