...
@section('content')
    ...
    <div class="well well-sm">
        <a href="{{route('g_itemnotice_add',['item_id'=>$edata['item_id']])}}" class="btn">添加通知</a>
    </div>
            ...
            @if($code=='success')
                @foreach($sdata as $infos)
                    <tr>
                        ...
                        <td>{{$infos->infos}}</td>
                        <td>
                            <a href="{{route('g_itemnotice_info',['id'=>$infos->id,'item_id'=>$infos->item_id])}}" class="btn btn-sm">查看详情</a>
                        </td>
                    </tr>
                @endforeach
            @endif
