...
<div class="wrap">
    ...
    <div class="center_text" style="margin-top: 440px;">征收单位:
        <div style="display: inline-block;width: 460px;height: 44px;border-bottom:3px solid #000;text-align: center;">秦州区房屋征收补偿管理局</div>
    </div>
    <div class="center_text" style="margin-top: 140px;">被征收人:
        <div style="display: inline-block;width: 460px;height: 44px;border-bottom:3px solid #000;text-align: center;">
            {{$holder->name}}
        </div>
    </div>
...
<div class="content" style="width: 794px;margin: auto;">
    <h2 class="center_text" style="line-height: 120px;">房屋征收补偿安置协议书(二)</h2>
    <div>
    ...
    <div>
        ...
        <div class="text_indent50 lineheight50 color66">
            <div class="text_indent50 lineheight50 ">
                乙方选择以下的<div class="width100 height40 div_border_bot">“{{$pay->transit_way}}”</div>临时安置方式
            </div>
            <ul>
                ...
                @if(filled($pay_transits))
                    ...
                    @foreach($pay_transits as $transit)
                        <div class="lineheight50 color66">
                            ({{$loop->iteration}})
                            <div class="width100 height40 div_border_bot">{{$transit->house->housecommunity->name}}</div>小区 ，房号
                            <div class="width100 height40 div_border_bot">
                                {{$transit->house->building}}-
                                {{$transit->house->unit}}-
                                {{$transit->house->floor}}-
                                {{$transit->house->number}}
                            </div>
                            面积<div class="width100 height40 div_border_bot">{{$transit->house->area}}</div>㎡。
                        </div>
                    @endforeach
                @endif
            @endif
                ...
                @switch($subject->subject_id)
                    ...
                    <div>
                        <div class="color66">临时安置费=被征收房屋合法建筑面积×{{$transit_price}}元/㎡×实际过渡月份（每月补偿不足{{$program->transit_base}}元的，按{{$program->transit_base}}元补偿）。选择自行过渡的临时安置费，自
                            <div class="width100 height40 div_border_bot">{{date('Y年m月d日',strtotime($sign_date))}}</div>（含当日）至
                            <div class="width100 height40 div_border_bot">{{date('Y年m月d日',strtotime($sign_date,'+ '.$transit_times.' month'))}}</div>（含当日）止，一次性发放；选择现房安置的，一次性发放{{$program->transit_real}}个月的临时安置费。</div>
                        <div class="color66">经核定，乙方应得临时安置费：
                            <div class="width100 height40 div_border_bot">{{number_format($legal->legal_area,2)}}</div>㎡×
                            <div class="width100 height40 div_border_bot">{{$transit_price}}</div>元/㎡×
                            <div class="width100 height40 div_border_bot">{{$transit_times}}</div>月（实际过渡月数）=
                            <div class="width100 height40 div_border_bot">{{number_format($subject->amount,2)}}</div>元（大写：
                            <div class="width100 height40 div_border_bot">{{bigRMB($subject->amount)}}</div>）。
                            @if($household->getOriginal('type'))
                                公房单位承租人占{{$program->portion_renter}}%，乙方实际应得临时安置费：
                                <div class="width100 height40 div_border_bot">{{number_format($subject->total,2)}}</div>元（大写：
                                <div class="width100 height40 div_border_bot">{{bigRMB($subject->total)}}</div>）。
                            @endif
                        </div>
                    </div>
                    @break
                    ...
                    <table class="table_a color66" border="1" cellspacing="0" style="width: 100%;page-break-before: auto;">
                        ...
                        <tr style="line-height: 80px;">
                            <th style="line-height: 60px;">被征收人</th>
                            <th style="line-height: 60px;">类别</th>
                            <th style="line-height: 60px;">上浮率（%）</th>
                            <th style="line-height: 60px;">上浮金额 (元)</th>
                        </tr>
                        </thead>
                        <tbody>
                        @if(filled($pay_crowds))
                            @foreach($pay_crowds as $crowd)
                                <tr>
                                    <td style="min-height: 80px;">{{$crowd->membercrowd->member->name}}</td>
                                    <td style="min-height: 80px;">{{$crowd->crowd->name}}</td>
                                    <td style="min-height: 80px;">{{$crowd->rate}}</td>
                                    <td style="min-height: 80px;">{{number_format($crowd->amount,2)}}</td>
                                </tr>
                            @endforeach
                    ...
                    <div class="text_indent50 lineheight50 color66">
                        经核定，乙方应得临时安置费特殊人群优惠补助共计<div class="width200 height40 div_border_bot">{{number_format($subject->total,2)}}</div> 元
                    ...
                    @case(15)
                    <div class="text_indent50 lineheight50 color66">
                        经核定，乙方应得搬迁奖励共计<div class="width200 height40 div_border_bot">{{number_format($subject->total,2)}}</div> 元
                        （大写：<div class="width200 height40 div_border_bot">{{bigRMB($subject->total)}}</div>）。
                    </div>
                    @break
                    ...
                    <div class="text_indent50 lineheight50 color66">
                        经核定，乙方合法房屋面积为
                        <div class="width200 height40 div_border_bot">{{number_format($legal->legal_area,2)}}</div> 平方米，
                        每平方米就得未新建、改建、扩建房屋奖励
                        <div class="width200 height40 div_border_bot">{{number_format($program->reward_real,2)}}</div> 元
                        乙方就得奖励
                        <div class="width200 height40 div_border_bot">{{number_format($subject->total,2)}}</div> 元
                        （大写：<div class="width200 height40 div_border_bot">{{bigRMB($subject->total)}}</div>）。
                    </div>
                    @break
                @endswitch
