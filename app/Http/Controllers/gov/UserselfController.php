...
class UserselfController extends BaseauthController
{
    ...
    public function index(Request $request){
        ...
        DB::beginTransaction();
        $user=User::with(['dept'=>function($query){
    ...
    public function edit(Request $request){
        ...
        /* ********** 保存 ********** */
            ...
            $model=new User();
            $rules=[
                'username'=>['required','alpha_num','between:4,20','unique:user,username,'.session('gov_user.user_id').',id'],
                'name'=>'required|min:2',
            ...
            try{
                /* ++++++++++ 锁定数据模型 ++++++++++ */
                $user=User::withTrashed()->where('secret',session('gov_user.secret'))->lockForUpdate()->first();
                if(blank($user)){
    ...
    public function password(Request $request){
        ...
        /* ********** 更新 ********** */
            ...
            try{
                /* ++++++++++ 锁定数据模型 ++++++++++ */
                $user=User::select(['id','password','secret'])->where('secret',session('gov_user.secret'))->lockForUpdate()->first();
                if(blank($user)){
                    throw new \Exception('数据不存在',404404);
                }
